...
export function RejectionShield({
  className,
  status = "safe",
  rejectionRate = 12.4,
}: RejectionShieldProps) {
  const stats = [
  ...
  const StatusIcon =
    status === "safe" ? ShieldCheck : status === "alert" ? Shield : ShieldAlert;

  return (
    <Card className={cn("p-lg space-y-lg", className)}>
      ...
      <div
        className={cn(
          "flex flex-col items-center justify-center py-xl px-12 border rounded-xl transition-all duration-700",
          statusColors[status],
        )}
      >
      ...
      <div className="grid grid-cols-1 gap-md">
        {stats.map((stat, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between p-md bg-background/50 rounded-md border border-border"
          >
            <span className="text-xs text-muted-foreground uppercase font-sans tracking-wide">
              {stat.label}
            </span>
      ...
      <div className="pt-md border-t border-border/50">
        ...
        <div className="grid grid-cols-6 gap-sm h-6">
          {[0.8, 0.9, 0.4, 0.7, 0.6, 0.8].map((val, i) => (
            <div
              key={i}
              className="bg-muted rounded-t-sm flex flex-col justify-end overflow-hidden"
            >
              <div
                className={cn(
                  "w-full transition-all duration-1000",
                  i === 2 ? "bg-neon-crimson" : "bg-neon-emerald",
                )}
