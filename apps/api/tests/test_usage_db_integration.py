...
class TestRecordUsagePersistence:
    ...
    async def test_1_7_db_004_record_usage_persists_row(
        ...
        result = await tenant_a_session.execute(
            text(
                "SELECT resource_type, resource_id, action, metadata_json "
                "FROM usage_logs WHERE resource_id = 'call_db_001'"
            )
        )
        row = result.first()
    ...
    async def test_1_7_db_005_trigger_sets_org_id(self, admin_session):
        ...
        await record_usage(
            ...
            ORG_A,
            resource_type="call",
            resource_id="call_db_002",
            action="call_initiated",
        )
        await admin_session.commit()

        result = await admin_session.execute(
            text("SELECT org_id FROM usage_logs WHERE resource_id = 'call_db_002'")
        )
    ...
    async def test_1_7_db_006_metadata_defaults_to_empty_json(self, admin_session):
        ...
        result = await admin_session.execute(
            text(
                "SELECT metadata_json FROM usage_logs WHERE resource_id = 'call_db_003'"
            )
        )
...
class TestMonthlyUsageCounting:
    ...
    async def test_1_7_db_007_counts_only_call_initiated(self, admin_session):
        ...
        result = await admin_session.execute(
            text(
                ...
                "WHERE action = 'call_initiated' AND org_id = :org_id "
...
class TestCheckUsageCapThresholds:
    ...
    async def test_1_7_db_010_warning_at_80_percent(self, admin_session):
        ...
        used = await get_monthly_usage(admin_session, ORG_A)
        threshold = _compute_threshold(used, 1000)
